namespace ChronoWiz.Ui.AboutHelp;
...
public partial class AboutHelp : ContentPage
{
	...
	{
		_ = await Browser.Default.OpenAsync(new Uri("http://eksit.dk/users-guide-3/"), BrowserLaunchMode.SystemPreferred);
	}

	private async void MyUrlButton_Clicked(object sender, EventArgs e)
	{
		_ = await Browser.Default.OpenAsync(new Uri("http://eksit.dk/"), BrowserLaunchMode.SystemPreferred);
	}

	private async void EmaiBtn_Clicked(object sender, EventArgs e)
	{
		_ = await Browser.Default.OpenAsync(new Uri("mailto:eks@eksit.dk"), BrowserLaunchMode.SystemPreferred);
	}
}
