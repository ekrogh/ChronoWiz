...
public sealed class MainWindowViewModel : ViewModelBase
{
	...
	{
		Navigation = new NavigationService(NavigateTo);
		...
		CalcCombinedCommand = new RelayCommand(() => ResultText = "Combined-calc not yet ported.");
		CalcStartCommand = new RelayCommand(() => ResultText = "Start-calc not yet ported.");
		CalcEndCommand = new RelayCommand(() => ResultText = "End-calc not yet ported.");
	...
	private void NavigateTo(string route)
	{
		...
		{
			RouteMain => this,
			RouteHelp => new AboutHelpViewModel(),
			RouteOpenIcs => new OpenIcsViewModel(),
			RouteSaveIcs => new SaveToIcsViewModel(),
			RouteFileIcs => new FileIcsViewModel(),
			_ => this
		};
	}
	...
	private void Calculate()
	{
		...
		var span = end - start;
		ResultText = $"Span: {span.TotalDays:0} days ({span.TotalHours:0} hours)";
	}
